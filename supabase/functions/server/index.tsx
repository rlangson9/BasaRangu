...
app.post("/make-server-5ed51d91/auth/send-otp", async (c) => {
  try {
    ...
    return c.json({ success: true, otp }); // In production, don't return OTP
  } catch (error) {
...
app.get("/make-server-5ed51d91/jobs", async (c) => {
  try {
    const type = c.req.query("type"); // "service", "errand", "recruitment"
    const category = c.req.query("category");
    const status = c.req.query("status") || "open";
    
    const allKeys = await kv.getByPrefix("job:");
    const jobs = allKeys.map(item => JSON.parse(item)).filter(job => {
...
app.post("/make-server-5ed51d91/jobs/:id/complete", async (c) => {
  try {
    ...
    if (job.paymentId) {
      ...
      const commission = payment.amount * 0.15; // 15% platform fee
...
app.post("/make-server-5ed51d91/reviews", async (c) => {
  try {
    ...
    const userReviews = reviews.map(r => JSON.parse(r)).filter((r: any) => r.targetUserId === targetUserId);
...
app.post("/make-server-5ed51d91/chat/:jobId/messages", async (c) => {
  try {
    ...
    const hasForbidden = forbidden.some(word => text.toLowerCase().includes(word));
    
    if (hasForbidden) {
      return c.json({ error: "Message contains forbidden contact information" }, 400);
    }
...
app.get("/make-server-5ed51d91/admin/disputes", async (c) => {
  try {
    ...
    const parsedDisputes = disputes.map(d => JSON.parse(d));
...
// Handle Pese Pay webhook notifications
app.post("/make-server-5ed51d91/pesepay/webhook", async (c) => {
  try {
    const webhookData = await c.req.json();
    
    console.log("Pese Pay webhook received:", webhookData);
    ...
    const payment = payments
      .map(p => JSON.parse(p))
      .find((p: any) => p.reference === reference);
...
app.get("/make-server-5ed51d91/admin/stats", async (c) => {
  try {
    ...
    return c.json({
      ...
      totalJobs,
      totalRevenue,
      activeJobs: jobs.filter(j => JSON.parse(j).status === "in_progress").length
    });
  } catch (error) {
